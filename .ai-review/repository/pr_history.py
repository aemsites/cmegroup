...
class PRHistory:
    ...
    def get_relevant_prs(self, file_path, limit=5):
        """Get relevant PR history for a file with discussion threads"""
        relevant_prs = []
                    ...
                    for pr in merged_prs:
                        ...
                        try:
                            ...
                            if modified_similar_files:
                                # Get comment threads for this PR
                                threads = self._get_comment_threads(pr)
                                
                                # Filter threads relevant to the modified files
                                file_threads = [
                                    t for t in threads 
                                    if t['original_comment']['file'] in modified_similar_files
                                ]
                                ...
                                context_prs.append({
                                    ...
                                    'state': pr.state,
                                    'merged': pr.merged,
                                    'context': f"From similar file: {', '.join(modified_similar_files)}",
                    ...
                    try:
                        ...
                        for file in files:
                            if file.filename == file_path:
                                ...
                                file_threads = [
                                    t for t in threads 
                                    if t['original_comment']['file'] == file_path
                                ]
                        ...
                        Log.print_yellow(f"Error processing PR: {str(e)}")
                        continue
