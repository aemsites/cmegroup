...
async function createFuturesContent() {
  const blocks = [];
  blocks.push('<p>Futures margins content</p>');
  return blocks;
}

async function createOptionsContent() {
  const blocks = [];
  blocks.push('<p>Options margins content</p>');
  return blocks;
}

// Main Content Functions
async function createMarginsContent(tabId, tabTitle, hasFuturesOptionsToggle) {
  const allBlocks = [];
...
export default async function buildMarginsTab(metadata = {}) {
  ...
  let blocks = [];
  try {
    blocks = await createMarginsContent(tabId, tabTitle, hasFuturesOptionsToggle);
  } catch (error) {
